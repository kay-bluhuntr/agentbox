...
tag=$(grep -E '^[[:space:]]+tag:' "$DEV_VALUES" | head -1 | sed -E 's/.*tag: *"?([^"]*)"?.*/\1/')
if [[ -z "$tag" || "$tag" == "initial" ]]; then
  echo "error: no built image tag found in $DEV_VALUES — has CI run on main yet?" >&2
  exit 1
fi

current=$(grep -E '^[[:space:]]+tag:' "$PROD_VALUES" | head -1 | sed -E 's/.*tag: *"?([^"]*)"?.*/\1/')
if [[ "$current" == "$tag" ]]; then
...
sed -i.bak -E "s|^([[:space:]]+tag:).*|\1 \"$tag\"  # promoted from dev via PR — see scripts/promote.sh|" "$PROD_VALUES"
